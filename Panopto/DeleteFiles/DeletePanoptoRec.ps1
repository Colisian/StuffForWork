...
$Action = New-ScheduledTaskAction -Execute "PowerShell.exe" -Execute "PowerShell.exe" -Argument "-ExecutionPolicy Bypass -Command `"$path = 'C:\PanoptoRecorder'; $days = 7; Get-ChildItem -Path `$path -Recurse | Where-Object { `$_.LastWriteTime -lt (Get-Date).AddDays(-`$days) -and `$_.FullName -notmatch 'C:\\\PanoptoRecorder\\\eventLogs' -and `$_.FullName -notmatch 'C:\\\PanoptoRecorder\\\UCSUploads' } | Remove-Item -Force -Recurse -Confirm:`$false`""
