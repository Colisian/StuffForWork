...
function set-autologon {
    ...
    # Function to update password using net commands
    function Update-UserPasswordNetCommands {
        try {
            ...
            $wmicResult = cmd.exe /c "wmic useraccount where name='$user' set passwordexpires=false"
            if ($LASTEXITCODE -ne 0) {
                Write-Host "Password expiration setting failed" -ForegroundColor Yellow
                return $false
            }
...
Set-Autologon -user "Libraries" -pass "ThisNeedsToChange11" -AutoLogonCount "0"