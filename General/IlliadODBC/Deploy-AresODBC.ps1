# Save as: Deploy-AresODBC.ps1
# Deploys Ares ODBC configuration as User DSN
...
# Configuration
$dsnName = "AresLink"
$serverName = "LIBRWS020V.AD.UMD.EDU"
$database = "Ares"
$loginID = "AresLink"
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid Margin="24">
        ...
        <TextBlock Grid.Row="0"
                   Text="Ares ODBC Database Connection Setup"
                   FontSize="16"
                   FontWeight="SemiBold"
                   Foreground="#1A1A1A"
                   FontFamily="Segoe UI"/>
        ...
        <TextBlock Grid.Row="1"
                   Text="Enter the AresLink database password:"
                   FontSize="13"
                   Foreground="#444444"
                   FontFamily="Segoe UI"
                   Margin="0,16,0,4"/>
        ...
        <TextBlock Grid.Row="2"
                   Text="(Contact RSR Head or lib-helpdesk@umd.edu for password)"
                   FontSize="11"
                   Foreground="#888888"
                   FontFamily="Segoe UI"
                   Margin="0,0,0,8"/>
...
if ([string]::IsNullOrWhiteSpace($plainPassword)) {
    Show-MessageDialog -Message "Password is required to configure the ODBC connection." -Title "Ares ODBC Setup - Error" -Type "Error"
    exit 1
}
...
if (-not $officeArch) {
    $continueSetup = Show-ConfirmDialog -Message "Could not detect Office installation.`n`nDefaulting to 32-bit Office configuration.`n`nContinue with 32-bit setup?" -Title "Ares ODBC Setup - Warning"
...
3. Run this setup again"
    ...
    Show-MessageDialog -Message $errorMessage -Title "Ares ODBC Setup - Missing Driver" -Type "Error"
    exit 1
}
...
This may indicate a permissions issue or registry corruption."
    ...
    Show-MessageDialog -Message $errorMessage -Title "Ares ODBC Setup - Registry Error" -Type "Error"
    exit 1
}
...
The configuration could not be completed."
    ...
    Show-MessageDialog -Message $errorMessage -Title "Ares ODBC Setup - Configuration Error" -Type "Error"
...
$successMessage = "Ares ODBC connection configured successfully!
...
Show-MessageDialog -Message $successMessage -Title "Ares ODBC Setup - Complete" -Type "Info"
