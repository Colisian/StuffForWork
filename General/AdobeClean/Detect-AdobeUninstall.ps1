...
.DESCRIPTION
    ...
    Keep $KeepPattern in sync with the -KeepPattern used when deploying.
...
$completed = (Get-ItemProperty -Path $SentinelKey -Name 'Completed' -ErrorAction SilentlyContinue).Completed
if ($completed -ne 1) {
    Write-Output "Not detected: sentinel Completed=$completed"
    exit 1
}
