...
**Fifth live run (v1.6.1-fullwipe, 2026-09-17, `LIBRWKSPC248869`).** CC reported FAILED after `exit 0 after 1 s` and a 15-second grace wait. The cause was a bug in the blocker check, not the uninstaller: Adobe states that [you can uninstall the Creative Cloud desktop app only when you have uninstalled all Creative Cloud apps](https://helpx.adobe.com/download-install/apps/manage-installed-apps/uninstall-creative-cloud-desktop-app.html), and this device still had **six legacy PDApp products** (Fireworks CS6, Flash Builder 4.7, Lightroom 6.7, SpeedGrade CC 2015, Extension Manager CC, ExtendScript Toolkit CC). `$ccBlockers` filtered on `Type -eq 'HD'` only, so it counted zero blockers, ran a doomed uninstaller, and reported the decline as a launcher handoff. Exit 0 in ~1 s with **no background child** is a decline, not the async pattern from v1.5.1 — the grace window correctly waited and correctly gave up.

v1.6.2 counts **any** non-CC, non-wrapper Adobe product as a blocker, so the step now refuses up front and names what is in the way. It also adds **STEP 5b**: PDApp-based CS6/CC2015 apps have no silent uninstall, so if `AdobeCreativeCloudCleanerTool.exe` is bundled beside the script (or `-CleanerToolPath` is given) a full-wipe run calls [`--removeAll=ALL --eulaAccepted=1`](https://helpx.adobe.com/id_en/enterprise/kb/cc-cleaner-tool-for-enterprise-users.html) before STEP 6. That flag also removes the CC desktop app and the Adobe ID credentials, so it is gated to `-RemoveCreativeCloud` runs. Without the tool, the step logs a WARN naming the legacy apps as blockers.

This run also exposed a cosmetic lie: STEP 8 logged `Cleared: C:\ProgramData\Adobe` when every child was on the preserve list and nothing was deleted. It now says so plainly.
...
- STEP 8 no longer logs `Cleared:` when everything was preserved and nothing was removed.
- `Uninstall-AdobeProducts-FullWipe.ps1` carries 5b; the main script gets the blocker and logging fixes only (paste budget).
