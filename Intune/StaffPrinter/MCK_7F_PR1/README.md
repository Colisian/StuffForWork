...
status: Ready
---
# MCK_7F_PR1 - Direct IP Printer (Intune Win32)
...
| Port | `IP_MCK_7F_PR1.resource.dyn.umd.edu` -> `MCK_7F_PR1.resource.dyn.umd.edu`:9100 (RAW) |
| Location | McKeldin - HR 7109 |
| Current print-server target | `10.204.152.28` |

## Package
```powershell
.\_Build\New-StaffPrinterPackages.ps1 -IntuneWinAppUtil <path>\IntuneWinAppUtil.exe -Name MCK_7F_PR1
```

## Company Portal (App information)
| Setting | Value |
|---|---|
| Name | `Printer - McKeldin - HR 7109 - Color (MCK_7F_PR1)` |
| Description | `Adds the MCK_7F_PR1 staff printer (McKeldin - HR 7109) to this computer for all users. Prints directly to the device - no print server.` |
| Publisher | `UMD Libraries IT` |
| Category | `Printers` |
| Show as featured app | No |
| Logo | `_Build\printer-icon.png` (if added) |
| Notes | `Canon iR-ADV C3935i - MCK_7F_PR1.resource.dyn.umd.edu` |
...
| Detection | Custom script: `Detect-MCK_7F_PR1.ps1` (32-bit: No, enforce signature: No) |
...
```powershell
Get-Printer -Name 'MCK_7F_PR1' | Format-List Name, DriverName, PortName, Location
Get-PrinterPort -Name 'IP_MCK_7F_PR1.resource.dyn.umd.edu' | Format-List Name, PrinterHostAddress, PortNumber
Resolve-DnsName 'MCK_7F_PR1.resource.dyn.umd.edu'
Test-NetConnection 'MCK_7F_PR1.resource.dyn.umd.edu' -Port 9100
Get-Content 'C:\ProgramData\StaffPrinters\MCK_7F_PR1-Install.log' -Tail 30
```
