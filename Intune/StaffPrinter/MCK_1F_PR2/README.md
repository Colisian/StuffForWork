...
## Package
```powershell
.\_Build\New-StaffPrinterPackages.ps1 -IntuneWinAppUtil <path>\IntuneWinAppUtil.exe -Name MCK_1F_PR2
```

## Company Portal (App information)
| Setting | Value |
|---|---|
| Name | `Printer - McKeldin - 1138 - Color (MCK_1F_PR2)` |
| Description | `Adds the MCK_1F_PR2 staff printer (McKeldin - 1138) to this computer for all users. Prints directly to the device - no print server.` |
| Publisher | `UMD Libraries IT` |
| Category | `Printers` |
| Show as featured app | No |
| Logo | `_Build\printer-icon.png` (if added) |
| Notes | `Canon iR-ADV C3935i - MCK_1F_PR2.resource.dyn.umd.edu` |
