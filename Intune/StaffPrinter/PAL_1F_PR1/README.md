...
## Package
```powershell
.\_Build\New-StaffPrinterPackages.ps1 -IntuneWinAppUtil <path>\IntuneWinAppUtil.exe -Name PAL_1F_PR1
```

## Company Portal (App information)
| Setting | Value |
|---|---|
| Name | `Printer - Performing Arts - Behind Circ Desk - B&W (PAL_1F_PR1)` |
| Description | `Adds the PAL_1F_PR1 staff printer (Performing Arts - Behind Circ Desk) to this computer for all users. Prints directly to the device - no print server.` |
| Publisher | `UMD Libraries IT` |
| Category | `Printers` |
| Show as featured app | No |
| Logo | `_Build\printer-icon.png` (if added) |
| Notes | `Canon iR-ADV 4925i - PAL_1F_PR1.resource.dyn.umd.edu` |
