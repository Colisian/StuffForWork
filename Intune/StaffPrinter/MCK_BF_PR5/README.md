...
## Package
```powershell
.\_Build\New-StaffPrinterPackages.ps1 -IntuneWinAppUtil <path>\IntuneWinAppUtil.exe -Name MCK_BF_PR5
```

## Company Portal (App information)
| Setting | Value |
|---|---|
| Name | `Printer - McKeldin - B0236 - B&W (MCK_BF_PR5)` |
| Description | `Adds the MCK_BF_PR5 staff printer (McKeldin - B0236) to this computer for all users. Prints directly to the device - no print server.` |
| Publisher | `UMD Libraries IT` |
| Category | `Printers` |
| Show as featured app | No |
| Logo | `_Build\printer-icon.png` (if added) |
| Notes | `Canon iR-ADV 4925i - MCK_BF_PR5.resource.dyn.umd.edu` |
