...
status: Ready
---
# MCK_2F_PR2 - Direct IP Printer (Intune Win32)
...
| Port | `IP_MCK_2F_PR2.resource.dyn.umd.edu` -> `MCK_2F_PR2.resource.dyn.umd.edu`:9100 (RAW) |
| Location | McKeldin - 2200B |
| Current print-server target | `10.204.152.18` |

## Package
```powershell
.\_Build\New-StaffPrinterPackages.ps1 -IntuneWinAppUtil <path>\IntuneWinAppUtil.exe -Name MCK_2F_PR2
```

## Company Portal (App information)
| Setting | Value |
|---|---|
| Name | `Printer - McKeldin - 2200B - Color (MCK_2F_PR2)` |
| Description | `Adds the MCK_2F_PR2 staff printer (McKeldin - 2200B) to this computer for all users. Prints directly to the device - no print server.` |
| Publisher | `UMD Libraries IT` |
| Category | `Printers` |
| Show as featured app | No |
| Logo | `_Build\printer-icon.png` (if added) |
| Notes | `Canon iR-ADV C3935i - MCK_2F_PR2.resource.dyn.umd.edu` |
...
| Detection | Custom script: `Detect-MCK_2F_PR2.ps1` (32-bit: No, enforce signature: No) |
...
```powershell
Get-Printer -Name 'MCK_2F_PR2' | Format-List Name, DriverName, PortName, Location
Get-PrinterPort -Name 'IP_MCK_2F_PR2.resource.dyn.umd.edu' | Format-List Name, PrinterHostAddress, PortNumber
Resolve-DnsName 'MCK_2F_PR2.resource.dyn.umd.edu'
Test-NetConnection 'MCK_2F_PR2.resource.dyn.umd.edu' -Port 9100
Get-Content 'C:\ProgramData\StaffPrinters\MCK_2F_PR2-Install.log' -Tail 30
```
