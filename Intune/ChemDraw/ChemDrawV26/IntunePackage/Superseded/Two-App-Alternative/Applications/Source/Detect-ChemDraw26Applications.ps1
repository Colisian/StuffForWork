<#
.SYNOPSIS Detects ChemDraw Applications 26.0.0 and completed bulk activation.
...
process {
    ...
    if ($core -and $apps64 -and $x86Satisfied -and $state.ApplicationsVersion -eq '26.0.0' -and $state.ActivationStatus -eq 'Activated') {
        Write-Output 'ChemDraw Applications 26.0.0 installed; suite activation completed.'
        exit 0
    }
    Write-Output 'ChemDraw Applications 26.0.0 or suite activation is missing.'
    exit 1
}
