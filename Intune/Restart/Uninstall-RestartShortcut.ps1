<#
.SYNOPSIS
    Removes the "RESTART" shortcut from the Public Desktop.

.DESCRIPTION
    Intune Win32 uninstall script. Runs non-interactively as SYSTEM.
    Deletes RESTART.lnk from the Public Desktop. Succeeds (exit 0) if the
...
.NOTES
    ...
    Date    : 2026-07-27
    Version : 1.0.0
    Log     : C:\ProgramData\RestartShortcut\Uninstall-RestartShortcut.log
...
Start-Transcript -Path (Join-Path $logDir 'Uninstall-RestartShortcut.log') -Append
