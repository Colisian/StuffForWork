<#  Install-Mck2FloorWideFormat.ps1
...
# ---- Config ----
$AppName     = 'Mck2FloorWideFormat_x64'
$ExeName     = 'LIB-Mck2FloorWideFormat_for_x64.exe'
...
try {
    ...
    # Strip Mark of the Web (Zone.Identifier). The vendor EXE is unsigned, so if
    # it carries ZoneId=3 the shell shows an "unverified publisher" prompt. That
    # prompt cannot be displayed under SYSTEM in session 0 - it would hang here.
    Unblock-File -LiteralPath $ExePath -ErrorAction SilentlyContinue

    # Launch installer.
    # -NoNewWindow (CreateProcess) rather than -WindowStyle Hidden (ShellExecute):
    # ShellExecute is what triggers the attachment-security prompt. The two
    # parameters are mutually exclusive, so this replaces -WindowStyle.
    Write-Log "Executing: `"$ExePath`" $ExeParams"
    $p = Start-Process -FilePath $ExePath -ArgumentList $ExeParams -Wait -PassThru -NoNewWindow
